...
import requests #open a webpage in get raw HTML data
...
def scrape_poll_data():
    ...
    soup = BeautifulSoup(html_data.content, 'html.parser') #specify beautifulsoup obj
    #print(soup.prettify())
    ...
    pollster_data_array = []
    for r in rows:
        date = r.find(class_='date-wrapper').text #find class and get text

        pollster = r.find(class_='pollster-container')
        pollster_text = pollster.find_all("a")[-1].text #find the anchor of the class
